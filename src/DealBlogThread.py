...
class DealBlogThread:
    ...
    def __init__(self, pageUrl = "", pageSize = 0):
        ...
        self.DealThread     =     threading.Thread(name = "DealBlog", target = self.DealBlogFunction)
        self.DealStopped = False
    ...
    def DealBlogFunction(self):
        ...
        #  获取到博客页面的信息
        #---------------------
        self.GetBlogPage( )            # 检索出所有的博客列表页面
        self.GetBlogUrl( )             # 获取到每个博客的页面信息
        self.ListBlogs( )
    ...
    def ListBlogs(self):
        ...
        """
        print "--------------------------------------------------"
        print u"共计发现博客 %d 篇" % (len(self.blogs) + len(self.noneBlogs))
        print u"待刷新博客 %d 篇" % (len(self.blogs))
        print u"拒绝刷新博客 %d 篇" %(len(self.noneBlogs))
#       print u"永不刷新博客 %d 篇" %(len(self.unflushList))
        print "--------------------------------------------------"


    def Start(self) :
        print "线程", self.DealThread.name, "启动..."
